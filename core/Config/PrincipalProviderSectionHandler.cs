...
namespace CrystalWall.Config
{
    ...
    public class PrincipalProviderSectionHandler : IConfigurationSectionHandler
    {
        public object Create(object parent, object configContext, System.Xml.XmlNode section)
        {
            ...
            try
            {
                if (section.Attributes["assembly"] == null)
                {
                    ...
                    {
                        ServiceManager.LoggingService.Error("当前运行的程序集中无法加载提供者", e);
                    }
                    try
                    {
                    ...
                    {
                        ServiceManager.LoggingService.Error("指定的提供者" + section.Attributes["class"].Value + "不是默认的提供者，将使用全限定名加载", e);
                    }
        ...
        init: provider.InitData(section, null, null);
            return provider;
        }
    }
}
